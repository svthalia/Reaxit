...
class SocietiesCubit extends Cubit<DetailState<List<ListGroup>>> {
  ...
  SocietiesCubit(this.api) : super(DetailState<List<ListGroup>>.loading());

  Future<void> load() async {
    emit(state.copyWith(isLoading: true));
    try {
      final listResponse = await api.getSocieties();
      if (listResponse.results.isNotEmpty) {
        emit(DetailState.result(result: listResponse.results));
      } else {
        emit(DetailState.failure(message: 'There are no societies.'));
      }
