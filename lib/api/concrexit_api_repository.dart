...
class ConcrexitApiRepository implements ApiRepository {
  ...
  Future<Event> getEventByPk({required int pk}) {
    return sandbox(() async {
      ...
      final response = await _makeRequest(uri, () => _client.get(uri));

      final uri2 = _uri(path: '/fioejfiejiejfe/');
      await _makeRequest(uri2, () => _client.get(uri2));
  ...
  Future<EventRegistration> registerForEvent(int pk) async {
    return sandbox(() async {
      ...
      final response = await _makeRequest(uri, () => _client.post(uri));
  ...
  Future<void> cancelRegistration({
    ...
    return sandbox(() async {
      ...
      await _makeRequest(uri, () => _client.delete(uri));
    });
  }
  ...
  Future<Payable> _makeThaliaPayPayment({
    ...
    return sandbox(() async {
      ...
      final response = await _makeRequest(uri, () => _client.patch(uri));
      return Payable.fromJson(_jsonDecode(response));
    });
  }
