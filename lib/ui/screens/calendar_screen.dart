  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: NetworkScrollableWrapper<EventsProvider>(
        builder: (context, events, child) => Container(
          ...
          child: Column(
            ...
            children: groupByMonth(events.eventList)
                .entries
                .map(
                  (monthGroup) => CalendarMonthCard(
                    monthGroup.key,
                    groupByDate(monthGroup.value)
                        .entries
                        .map((dayGroup) => CalendarDayCard(DateFormat(DateFormat.ABBR_WEEKDAY).format(dayGroup.key), dayGroup.key.day,
